...
async function handleDownload() {
    ...
    try {
        ...
        // Add progress items for each job
        ...
        // Start polling for jobs without WebSocket support
        response.jobs.forEach(job => {
...
async function pollProgress(jobId) {
    let attempts = 0;
    const maxAttempts = 600; // 10 minutes max (1 second interval)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    APIClient.health().then(() => {
        ...
        UI.showError('Cannot connect to server. Please ensure the backend is running.');
    });
});
